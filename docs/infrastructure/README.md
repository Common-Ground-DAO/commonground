...
- **Stage 0 (`Dockerfile_dev_stage_0`):** `FROM node:24.18-bookworm`. Installs system deps, copies `package.json` / `yarn.lock` / `.yarnrc.yml`, enables Corepack and runs `yarn`; prunes the darwin/win32 binaries out of `onnxruntime-node` (~160 MB that can never run in the linux image). A separate layer then runs `download_model.sh`, which fetches the NSFW classification model (~87 MB) from a **pinned HuggingFace revision** into `/models/nsfw` and verifies every file's sha256 — the build needs network access to `huggingface.co`, the runtime never does. Cached; rebuilt only when dependencies (or the model pin) change.
...
| `IMAGE_MODERATION_ENABLED` | Server-side NSFW image filter (`srv/moderation/imageFilter.ts`); consumed by the `api` and `onchain` services. Default `true`; only the literal `false` disables it. |
| `IMAGE_MODERATION_THRESHOLD` | NSFW probability above which an image is rejected. Default `0.8`; invalid/empty values fall back to the default. |
