...
| Ecosystems (`EcosystemProvider`, `/e/:ecosystem`, `EcosystemPicker`/`Chip`/`PickerField`/`HomeHeader`, hardcoded partner list, `ecosystemTagList`, partner icons) | **removed 2026-08-01** | Phase 3.5 of the slimming roadmap. **Tags are the replacement** — ecosystems were never anything but strings in `communities.tags`, and the backend never knew the concept. `/e/*` redirects to `/`; existing partner tags live on as ordinary tags (no data cleanup). LUKSO compatibility (UP login, LSP7/LSP8, chain config, `READ_LUKSO`) and every chain tag/icon are untouched. Do not reintroduce. |
