...
| `IMAGE_MODERATION_ENABLED` | Server-side NSFW image filter (`srv/moderation/imageFilter.ts`); consumed by the `api` and `onchain` services (with defaults when absent from `.env`). Default `true`; only the literal `false` disables it. Note: on the dev stack the *client* pre-check stays active regardless (no instance-config injection in dev); only the selfhost profile propagates the switch to the browser. |
| `IMAGE_MODERATION_THRESHOLD` | NSFW probability above which an image is rejected. Default `0.8`; invalid/empty values fall back to the default with a startup warning. |
