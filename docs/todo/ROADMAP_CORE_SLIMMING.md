...
- [x] Backend: `signableWalletDataValidator` is EVM-only, `prepareWalletAction` accepts `cg_evm`/`evm`, `FuelAddress`/`AeternityAddress` leave `validators/common.ts`, nginx drops `testnet.aeternity.io` / `mainnet.aeternity.io` / `testnet.fuel.network` from every CSP. **Finding:** `walletHelper.getLoginWalletUserId` (the other fuel/aeternity verification site) had **no callers at all** — wallet login goes through the prepared-credential session — so it was removed together with `_getLoginWalletByIdentifier`, its only helper. `errors.server.WALLET_NOT_ALLOWED_FOR_LOGIN` is orphaned by that removal and left in place (shared error table, harmless).
